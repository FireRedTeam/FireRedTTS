...
class ConditionalCFM(nn.Module):
    ...
    def solve_euler_fast(self, x, t_span, mu, mask):
        b = x.shape[0]
        t, _, dt = t_span[0], t_span[-1], t_span[1] - t_span[0]

        for step in range(1, len(t_span)):
            # conditional/unconditional in one run
            ...
            dphi_dt = (1.0 + self.inference_cfg_rate) * dphi_dt_joint[
                :b
            ] - self.inference_cfg_rate * dphi_dt_joint[b:]
            x = x + dt * dphi_dt
            t = t + dt
    ...
    def inference(self, mu, mask, n_timesteps, temperature: float = 1.0):
        ...
        return self.solve_euler_fast(z, t_span=t_span, mu=mu, mask=mask)
