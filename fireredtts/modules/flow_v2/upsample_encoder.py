...
class Upsample1D(nn.Module):
    ...
    def forward_chunk(
        ...
        if cache is None:
            cache = torch.zeros(
                inputs.shape[0], inputs.shape[1], self.stride * 2, device=inputs.device
            )
...
class PreLookaheadLayer(nn.Module):
    ...
    def forward_chunk(self, inputs: torch.Tensor, cache: torch.Tensor = None):
        ...
        if cache is None:
            cache = torch.zeros(
                outputs.shape[0], outputs.shape[1], 2, device=outputs.device
            )
        # NOTE
        new_cache = outputs[..., -2:]
        outputs = torch.cat([cache, outputs], dim=2)
...
class UpsampleConformerEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.up_layer = Upsample1D(
            channels=output_size, out_channels=output_size, stride=up_stride
        )
        # Input embedding2
    ...
    def forward_chunk(
        ...
        att_cache1: torch.Tensor = None,
        att_cache2: torch.Tensor = None,
    ):
        """We don't perform attention stripping because the relative positional encoding are hard to deal with.
        ...
        Args:
            ...
            att_cache1: shape (depth1+depth2, b, nh, t, c). Attention cache of the first Conformer(TOKEN-level).
            att_cache2: shape (depth1+depth2, b, nh, 2*t, c). Attention cache of the second Conformer(FEAT-level).
        """
        # unpack cnn_cache
        ...
        offset1 = att_cache1.shape[3]
        pos_emb = self.embed.position_encoding(
            offset=None, size=offset1 + xs.shape[1]
        )  # (b, 2*t-1, c)
        ...
        for idx, layer in enumerate(self.encoders):
            ...
            xs, _, this_new_att_cache1, _ = layer(
                xs, mask=None, pos_emb=pos_emb, att_cache=att_cache1[idx]
            )
        ...
        # remake pos_emb
        offset2 = att_cache2.shape[3]
        pos_emb = self.embed.position_encoding(offset=None, size=offset2 + xs.shape[1])

        # second conformer
        new_att_cache2 = []
        for idx, layer in enumerate(self.up_encoders):
            xs, _, this_new_att_cache2, _ = layer(
                xs, mask=None, pos_emb=pos_emb, att_cache=att_cache2[idx]
            )
        ...
        return xs, new_cnn_cache, new_att_cache1, new_att_cache2
